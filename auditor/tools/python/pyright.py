...
def _rel(root: Path, p: str | None) -> Optional[str]:
    ...
    try:
        sp = sp.resolve()
        return str(sp.relative_to(root.resolve()))
    except Exception:
        return str(sp)
...
class PyrightTool(AuditTool):
    ...
    def parse(self, result: ToolRunResult) -> List[Finding]:
        ...
        # If you need a summary Finding later, you can synthesize it here,
        # but omit severity and include metrics/tags similarly.
